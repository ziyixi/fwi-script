...
sampling_rate = 10
logfile = "/mnt/ls15/scratch/users/xiziyi/process_data/process_data_200.log"
RAW_DIR = "/mnt/research/seismolab2/japan_slab/data/asdf_raw_EARA2014"
PROCESSED_DIR = "/mnt/ls15/scratch/users/xiziyi/process_data/data_200"
...
def get_scripts(run_files):
    ...
    for iiter in range(N_iters):
        ...
        for ieach in range(N_node):
            ...
            result += f"srun -N 1 -n {N_cores_each_node} {PY} ../process/process_data_fromsac.py --min_periods {min_periods} --max_periods {max_periods} --asdf_filename {filename} --waveform_length {waveform_length} --sampling_rate {sampling_rate} --output_directory {PROCESSED_DIR} --logfile {logfile} --no-correct_cea --cea_correction_file {cea_correction_file} --paz_directory {paz_path} &"
        result += f"wait; "
