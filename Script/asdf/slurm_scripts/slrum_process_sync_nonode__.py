...
def get_scripts(run_files):
    ...
    for iiter in range(N_iters):
        ...
        for ieach in range(N_each):
            ...
            offset = iiter*N_each+ieach
            if(offset >= N_files):
                continue
            filename = run_files[offset]
            result += f"mpirun -n {N_each_cors} {PY} process_sync.py --min_periods {min_periods} --max_periods {max_periods} --asdf_filename {filename} --waveform_length {waveform_length} --sampling_rate {sampling_rate} --output_directory {PROCESSED_DIR} --logfile {logfile} &"
        result += f"wait; "
...
def submit_job(thecommand):
    s = Slurm("process_sync", {"ntasks": N_cores,
