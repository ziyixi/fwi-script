...
def get_scripts():
    ...
    for iiter in range(N_iters):
        ...
        for ieach in range(N_node):
            ...
            result += f"srun -N 1 -n {N_cores_each_node} {PY} ../get_info/get_flexwin_windows.py --obs_path {obs_path} --syn_path {syn_path} --out_path {out_path} --logfile {logfile} &"
        result += f"wait; "
...
def submit_job(thecommand):
    s = Slurm("flexwin", {"nodes": N_node, "ntasks": N_cores,
                          "time": "03:00:00", "cpus-per-task": 1, "mem-per-cpu": "2G"})
    s.run(thecommand)
