...
def main(obs_path, ref_path, pkl_path):
    ...
    with pyasdf.ASDFDataSet(obs_path, mode="a") as obs_ds:
        with pyasdf.ASDFDataSet(ref_path, mode="r") as ref_ds:
            ...
            # get a dict for stla and stlo
            stations = np.loadtxt("./stations.txt", dtype=str)
            stations_mapper = {}
            ...
            def process(sg_obs, sg_ref):
                ...
                # get stla and stlo
                station = sg_obs[tag_obs][0].stats.station
                network = sg_obs[tag_obs][0].stats.network
                stla, stlo = stations_mapper[(network, station)]
